...
:::caution Dynamic DB Connection Routing Availability

The Dynamic DB Connection Routing feature is currently available in `beta` only via the API and the CLI without any
usage limits in both the Cloud Professional and Cloud Enterprise plans. We will soon add the functionality to configure
dynamic database routing on the Console and, along with that, introduce usage limits for this feature in the Cloud
Professional plan. Cloud Enterprise customers will be able to continue using the feature without limits.
...
Console support will be added soon. Please use either the CLI or API for configuring dynamic DB connection routing.
